...
def create_report_data(self, user_id, municipality_id, address_line_1, address_line_2, city, zipcode, geo_data_lat,
                       geo_data_long, img_src):
    query = """INSERT INTO report_data (user_id, municipality_id, address_line_1, address_line_2, city, zipcode, geo_data_lat, geo_data_long, img_src)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
    params = (
        user_id, municipality_id, address_line_1, address_line_2, city, zipcode, geo_data_lat, geo_data_long, img_src)
...
def get_report_data_by_id(self, data_id):
    query = """SELECT * FROM report_data WHERE data_id = %s;"""
